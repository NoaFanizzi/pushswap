...
/*   Updated: 2025/02/27 15:00:47 by nofanizz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

static t_cheapest	ft_get_between_a(t_stack *stack, int i)
{
	t_cheapest	cheapest;
	int			nb;
	...
	{
		if (nb - stack->index_b[i] < cheapest.cost && (nb
				- stack->index_b[i]) > 0)
		{
			cheapest.cost = nb - stack->index_b[i];
			cheapest.pos.b = i;
...
static t_cheapest	get_cheapest_b(t_stack *stack, int i)
{
	int			nb;
	t_cheapest	cheapest;

	nb = stack->index_a[i];
	if (ft_is_new_biggest_a(stack, nb) == 1)
		cheapest = ft_get_biggest_a(stack, i);
	else if (ft_is_new_lowest_a(stack, nb) == 1)
		cheapest = ft_get_lowest_a(stack, i);
	else
		cheapest = ft_get_between_a(stack, i);
	return (cheapest);
}
...
t_cheapest	ft_cheapest_diff_a(t_stack *stack, int i)
{
	...
	i = 0;
	current = get_cheapest_b(stack, i);
